...
test('FIRMS retains a large successful source during partial failure and filters stale data at serve time', async (t) => {
  ...
  const csv = header + '-27.47,-58.83,2026-09-12,1200,h,10\n'.repeat(130001);
  t.mock.method(globalThis, 'fetch', async (raw) => {
    calls++;
    const url = new URL(raw);
    ...
    assert.match(url.pathname, /\/-74,-56,-52,-21\/2$/);
  ...
  assert.equal(first.region, 'Argentina');
  assert.deepEqual(first.bounds, [-74, -56, -52, -21]);
