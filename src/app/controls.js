...
import { flyToArgentina } from '../camera.js';
import { initCockpitCloudEffects } from '../cockpitCloudEffects.js';
import { createGroundExplorer } from '../ui/groundExplorer.js';
...
export function createApplicationControls({
  ...
  const groundExplorer = createGroundExplorer(viewer);
  defer(() => groundExplorer.destroy());

  // If no share link state, open on the national operating picture.
  if (!styleManager.hasShareState) {
    loaderStatus.textContent = 'Abriendo vista nacional de Argentina...';
    defer(flyToArgentina(viewer));
  } else {
    loaderStatus.textContent = 'Restaurando vista compartida...';
  }
