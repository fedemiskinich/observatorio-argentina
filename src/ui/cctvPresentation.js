...
export function _renderCctvState(state) {
  ...
  const embedUrl = enabled ? activeCamera?.embedUrl || '' : '';
  const showLiveWindow = Boolean(
    embedUrl && activeId !== this._dismissedEmbedCameraId,
  );
  if (this._cctvLiveWindow) this._cctvLiveWindow.hidden = !showLiveWindow;
  if (this._cctvLiveTitle) {
    this._cctvLiveTitle.textContent = activeCamera
      ? `${activeCamera.name} · ${activeCamera.city}`
      : 'Cámara en vivo';
  }
  if (this._cctvLiveSource) {
    const sourceUrl = activeCamera?.sourceUrl || '';
    this._cctvLiveSource.hidden = !sourceUrl;
    if (sourceUrl) this._cctvLiveSource.href = sourceUrl;
    else this._cctvLiveSource.removeAttribute('href');
  }
  if (this._cctvLiveFrame) {
    const currentLive = this._cctvLiveFrame.dataset.currentSrc || '';
    if (showLiveWindow && currentLive !== embedUrl) {
      this._cctvLiveFrame.src = embedUrl;
      this._cctvLiveFrame.dataset.currentSrc = embedUrl;
    } else if (!showLiveWindow && currentLive) {
      this._cctvLiveFrame.removeAttribute('src');
      this._cctvLiveFrame.dataset.currentSrc = '';
    }
  }
  if (this._cctvWebFrame) {
    const currentEmbed = this._cctvWebFrame.dataset.currentSrc || '';
    // Web feeds live in the dedicated floating player. Keeping a second
    // iframe inside the panel duplicates video/audio and can make providers
    // reject one of the two playback sessions.
    if (currentEmbed) {
      this._cctvWebFrame.removeAttribute('src');
      this._cctvWebFrame.dataset.currentSrc = '';
    }
    this._cctvWebFrame.classList.remove('active');
    this._cctvFrameWrap?.classList.toggle('has-web-feed', false);
  }

  if (this._cctvProjectionBtn && embedUrl) {
    this._cctvProjectionBtn.textContent = 'WEB PLAYER';
    this._cctvProjectionBtn.classList.remove('active');
    this._cctvProjectionBtn.disabled = true;
  }

  if (this._cctvFrame) {
    const nextSrc = enabled && !embedUrl ? activeCamera?.frameUrl : null;
