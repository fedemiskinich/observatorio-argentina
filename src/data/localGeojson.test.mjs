...
async function enableLayerWithFetch(fetchImpl, { dataSources, windowStub, layerOptions = {} } = {}) {
  const originalFetch = globalThis.fetch;
  const originalWindow = globalThis.window;
...
test('a dynamic WFS layer refreshes after the camera settles with a new extent', async () => {
  let extent = 'west';
  const requestedUrls = [];
  const response = {
    ok: true,
  ...
  const { viewer, cleanup } = await enableLayerWithFetch(
    async (requestedUrl) => {
    ...
    {
      layerOptions: {
        urlForViewer: () => `/ign.geojson?bbox=${extent}`,
        refreshOnMove: true,
  ...
  assert.deepEqual(requestedUrls, ['/ign.geojson?bbox=west']);
  extent = 'east';
  const originalFetch = globalThis.fetch;
  ...
  assert.deepEqual(requestedUrls, [
    '/ign.geojson?bbox=west',
    '/ign.geojson?bbox=east',
  ]);
  cleanup();
});
