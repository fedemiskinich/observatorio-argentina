...
export function createLocalGeoJsonLayer(
  ...
  const layer = {
    ...
    enable: async (viewer) => {
      ...
      const requestedUrl =
        typeof urlForViewer === 'function' ? urlForViewer(viewer) : url;
      if (!requestedUrl) {
        _error = 'dataset unavailable (no valid view extent)';
        return;
      }
      if (_loadedUrl && _loadedUrl !== requestedUrl) {
        _cachedFeatures = null;
        releaseDataSource(viewer);
      }
      _loadedUrl = requestedUrl;
          ...
          _loadPromise = (async () => {
            ...
            try {
              ...
              if (!features) {
                ...
                let parsed = null;
                try {
                  parsed = trimmed.startsWith('{') ? JSON.parse(trimmed) : null;
                } catch (error) {
                  if (!trimmed.includes('\n')) throw error;
                }
                features =
      ...
      if (_enabled && !_cameraMoveEndRemover) {
        _cameraMoveEndRemover = viewer.camera.moveEnd.addEventListener(() => {
          ...
          if (refreshOnMove && typeof urlForViewer === 'function') {
            if (_viewRefreshTimer) clearTimeout(_viewRefreshTimer);
            _viewRefreshTimer = setTimeout(() => {
              _viewRefreshTimer = null;
              if (!_enabled || _destroyed) return;
              void _enableAgain?.(viewer);
            }, Math.max(100, Number(refreshDebounceMs) || 700));
          }
        });
      }
...
function layerTitle(layerId) {
  ...
  if (layerId === 'ign-airports') return 'Aeropuerto';
  if (layerId === 'ign-ports') return 'Puerto';
  if (layerId === 'ign-localities') return 'Localidad';
  return 'Feature';
}
