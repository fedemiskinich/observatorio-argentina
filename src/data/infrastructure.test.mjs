...
test('infrastructure factory preserves identity and creates independent state without loading', (t) => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { id: 'ign-airports', name: 'Aeropuertos IGN', source: 'Instituto Geográfico Nacional · WFS' },
      { id: 'ign-ports', name: 'Puertos IGN', source: 'Instituto Geográfico Nacional · WFS' },
      { id: 'ign-localities', name: 'Localidades IGN', source: 'Instituto Geográfico Nacional · BAHRA · WFS' },
    ],
  );
...
test('IGN requests use the visible rectangle when it overlaps Argentina', () => {
  const viewer = {
    scene: { globe: { ellipsoid: {} } },
    camera: {
      computeViewRectangle: () => ({
        west: Cesium.Math.toRadians(-60),
        south: Cesium.Math.toRadians(-30),
        east: Cesium.Math.toRadians(-57),
        north: Cesium.Math.toRadians(-26),
      }),
    },
  };
  const bounds = ignBoundsForViewer(viewer);
  assert.ok(Math.abs(bounds.west + 60) < 1e-9);
  assert.ok(Math.abs(bounds.south + 30) < 1e-9);
  assert.ok(Math.abs(bounds.east + 57) < 1e-9);
  assert.ok(Math.abs(bounds.north + 26) < 1e-9);
});
