 ...
 * Argentina fork default: national context on load.
 */
export const CAMERA_PRESETS = {
  argentina: {
    destination: Cesium.Cartesian3.fromDegrees(-64.2, -38.4, 4200000),
    orientation: {
  ...
  corrientes: {
    destination: Cesium.Cartesian3.fromDegrees(-58.8341, -27.4692, 1200),
    orientation: {
  ...
  buenosaires: {
    destination: Cesium.Cartesian3.fromDegrees(-58.3816, -34.6037, 1400),
    orientation: {
      heading: Cesium.Math.toRadians(20),
      pitch: Cesium.Math.toRadians(-35),
      roll: 0.0,
...
/**
 * Present the national operating picture first, then settle over Argentina.
 * @returns {Function} Cancels the pending or active startup flight.
 */
export function flyToArgentina(viewer) {
  viewer.camera.setView({
    destination: Cesium.Cartesian3.fromDegrees(-64.2, -38.4, 9500000),
    orientation: CAMERA_PRESETS.argentina.orientation,
  });

  const timer = setTimeout(() => {
    if (viewer.isDestroyed()) return;
    viewer.camera.flyTo({
      destination: CAMERA_PRESETS.argentina.destination,
      orientation: CAMERA_PRESETS.argentina.orientation,
      duration: 4.0,
      easingFunction: Cesium.EasingFunction.CUBIC_IN_OUT,
    });
