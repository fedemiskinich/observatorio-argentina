...
test('markup, startup ordering and accessibility remain pinned', () => {
  ...
  assert.match(visible, /sismos/i);
  assert.match(visible, /incendios/i, 'the tile must promise the fires it enables');

  // The Argentina fork's public-data statement is pinned verbatim.
  assert.ok(
    html.includes(
      '<p id="first-run-description">Inteligencia geoespacial construida con fuentes abiertas, trazables y datos públicos.</p>',
    ),
    'the Argentina first-run line must ship exactly as written',
  );
...
// ── Voice: Argentina layers are part of the pinned tool schema ──────────

test('the voice TOOL SCHEMA matches the Argentina layer release', () => {
  // Argentina IGN layers deliberately extend the layer menus and entity context.
  ...
  assert.equal(block.length, 26340, 'serialized tool schema length drifted');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    '2067329032e6e1770851af4d3bb50a61b091a7c8bc7135150371c042dce0e1c8',
    'the Argentina layer tool schema must remain intentional and reviewable',
  );
