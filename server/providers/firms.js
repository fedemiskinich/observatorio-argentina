...
export function firmsProxy() {
  ...
  // Continental Argentina plus Tierra del Fuego. FIRMS expects
  // west,south,east,north in decimal degrees.
  const ARGENTINA_AREA = '-74,-56,-52,-21';
  ...
  const inArgentinaArea = (record) => {
    const lat = Number(record?.lat);
    const lon = Number(record?.lon);
    return lat >= -56 && lat <= -21 && lon >= -74 && lon <= -52;
  };
  ...
  async function fetchSource(key, source) {
    const url = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/${encodeURIComponent(key)}/${source}/${ARGENTINA_AREA}/2`;
  ...
  function buildPayload(entry, stale) {
    // Filter again at serve time so a disk cache created by an older global
    // build can never leak out-of-region detections after upgrading.
    const fires = filterTrailing24h(entry.fires, Date.now()).filter(inArgentinaArea);
    return {
      fetchedAt: entry.at,
      stale,
      ttlMs: TTL_MS,
