...
/** Foreign live packs are opt-in in the Argentina fork. */
const foreignLivePacksEnabled = () =>
  String(process.env.CCTV_LIVE_PACKS_ENABLED || '0').trim() === '1';
...
export function createCctvCatalog({ sourceRoot = process.cwd() } = {}) {
  ...
  async function refreshCctvSources() {
    ...
    const livePacksEnabled = foreignLivePacksEnabled();
